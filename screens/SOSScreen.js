...
const SOSScreen = ({ navigation }) => {
      ...
      useEffect(() => {
            ...
            return () => {
                  ...
                  // Do not stop tracking automatically on unmount if we want it to persist in background?
                  // For now, we will stop it when user explicitly exits or component unmounts to avoid leaks.
                  if (locationSubscription.current) {
                        locationSubscription.current.remove();
                  }
                  // Optionally delete session on unmount if not properly stopped? 
                  // Better to leave it or handle in stopTracking.
            };
      }, []);
      ...
      const handleSendSOS = async () => {
            ...
            try {
                  ...
                  console.error(error);
                  setIsSending(false);
                  // If failed, maybe stop tracking?
                  stopTracking();
            }
      };

      // Render Active Tracking UI
      if (isTracking) {
            return (
